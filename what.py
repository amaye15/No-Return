...
def MotionCloudNoise(sf_0=0.125, B_sf=3., alpha=.0, N_pic=128, seed=42, motion = False): # N_pic = 28
    if motion == True:
    ...
    if motion == True:
        z = z.reshape((mc.N_X, mc.N_Y,mc.N_frame)) 
    else:
        z.reshape((mc.N_X, mc.N_Y))
    return z, env
...
class WhatShift(object):
    ...
    def __call__(self, sample_index):
        ...
        sample = np.pad(np.array(sample_index[0]),(50,50), 'constant', constant_values=(0)) # np.array(sample_index[0])
        #print(sample)
...
class WhatBackground(object):
    ...
    def __call__(self, sample):
        ...
        #data = np.repeat(data,60).reshape(128,128,60) #added
        #im_noise = np.transpose(im_noise,(2,0,1))
        #print(im_noise.shape)
        #im = [np.max((data,im_noise[:,:,i]),axis=0) for i in range(0,59)]
        #im = [np.max((data,im_noise[:,:,i]),axis=0) for i in range(0,mc.N_frame-1)]
        #im = [np.max((data,im_noise[:,:,[i]]),axis=0) for i in range(0,60)]
        im = np.max((data,im_noise[:,:,0]), axis = 0)
        ...
        #temp2 = np.ones((128,128))
        #temp = np.dstack((im,im,im,temp2))
        #print(temp)
        
        #im = np.concatenate((im,im,np.ones(128,128)))
        
        #im = np.reshape(im,(128,128,4))
